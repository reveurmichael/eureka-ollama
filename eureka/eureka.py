...
def main(cfg):
    ...
    generator = OllamaGenerator(model=cfg.model)
    ...
    output_file = f"{ISAAC_ROOT_DIR}/tasks/{env_name}.py"
    ...
    create_task(ISAAC_ROOT_DIR, cfg.env.task, cfg.env.env_name, '')
    ...
    for iter in range(cfg.iteration):
        ...
        # Generate responses using OllamaGenerator
        responses = generator.generate(prompt, cfg.sample, logging, cfg.temperature)

        if cfg.sample == 1:
            logging.info(f"Iteration {iter}: Ollama Output:\n " + responses[0] + "\n")

        code_runs = [] 
        rl_runs = []
        for response_id, response_content in enumerate(responses):
